...
secondModel <- mapk_ode_robert2(initial_states, rates,
                                interventions = list(PRaf = 50, PPMek = 30))
secondModel()
...
dx <- function(x,y,activate,...) {
  x * y * activate
}

dy <- function(x,y,deactivate,...){
  x * deactivate / y
}
...
# with intervention
meta_model(model, list("dx","dy"), rates, states, list(x = 100))
...
test <- meta_model(c(dE1, dPRaf, dPPMek, dPPErk), c("dE1", "dPRaf", "dPPMek", "dPPErk"), rates, initial_states)
ode_out_meta_model <- ode_sim_meta_model(test, initial_states, times)

# meta_model(cdE1, dPRaf, dPPMek, dPPErk), c("dE1", "dPRaf", "dPPMek", "dPPErk"), rates, initial_states, list())
